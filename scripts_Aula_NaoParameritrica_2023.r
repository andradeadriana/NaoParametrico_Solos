...
setwd("G:/Meu Drive/_UFRRJ/Post Graduation/Solos/Não Paramétrico/2023/DataR")
...
boxplot(Peso~Tratamento)

#Teste Mann-Whitney

wilcox.test(Peso~Tratamento, data=Peso_Mann)
...
boxplot(data$colinesterase~ data$Tempo)
...
boxplot(Cultura_Kruskal$cultura~Cultura_Kruskal$grupo,
        col="gray", xlab="Grupo",ylab="Peso_Cultura",main="Peso da Cultura segundo Grupo de Tratamento")
...
library(dunn.test)
dunn.test(Cultura_Kruskal$cultura,Cultura_Kruskal$grupo,method="bonferroni",list=T)
...
boxplot(VacasFriedman$Producao~VacasFriedman$Trat,col="gray",main="Produção de leite",xlab="Tratamento",ylab = "Produ??o")


##Teste de Friedman

friedman.test(VacasFriedman$Producao,groups = VacasFriedman$Trat,blocks = VacasFriedman$Blocos)


##Teste de igualdade das distribuições - comparações múltiplas

pairwise.wilcox.test(VacasFriedman$Producao,VacasFriedman$Trat,p.adj="bon",paired = TRUE)

library(PMCMRplus)
 frdAllPairsConoverTest(VacasFriedman$Producao,groups = VacasFriedman$Trat,blocks = VacasFriedman$Blocos,
                               p.adjust.method = "bonferroni")

#Análise de Comparação Múltipla com a library ggpubr
#Criação de objeto com teste de comparação
pwc <- VacasFriedman %>%
  rstatix::wilcox_test(Producao ~ Trat,paired = TRUE, p.adjust.method = "bonferroni")
pwc
pwc2 <- VacasFriedman %>%
  rstatix::sign_test(Producao ~ Trat, paired = TRUE, p.adjust.method = "bonferroni")
pwc2  

#Gráfico com teste
x11()
pwc <- pwc %>% rstatix::add_xy_position(x = "Trat")
ggpubr::ggboxplot(data = VacasFriedman,x="Trat",y="Producao",
                  color = "Trat",
                  #notch = TRUE,
